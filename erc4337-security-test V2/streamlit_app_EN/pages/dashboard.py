...
def _render_recent_runs(root: Path) -> None:
    ...
    if not recent:
        st.info("No run records yet (streamlit_app/runtime/manifests is empty)")
        return
...
def render() -> None:
    ...
    st.write("Unified entry overview: environment status, deployment status, testing and analysis assets.")
